...
import { useState, useRef, useEffect, useOptimistic, useTransition, startTransition } from "react";
...
export default function ChatWindow({ conversation, onBack }) {
  ...
  // Get real messages from context
  const realMessages = getMessagesForConversation(conversation.id);

  // useTransition is REQUIRED for optimistic updates in React 19
  const [isPending, startTransition] = useTransition();

  // useOptimistic hook for immediate UI feedback
  ...
  // Use optimistic messages for rendering (shows immediately)
  ...
  const handleSend = async (e) => {
    ...
    const optimisticMessage = { 
      ...
      media_url: null, 
      isRead: false,
      pending: true // Flag to indicate this is an optimistic/pending message
    };

    // Wrap in startTransition - REQUIRED by React 19 for optimistic updates
    // This prevents the "An optimistic state update occurred outside a transition or action" error
    startTransition(async () => {
      // Add optimistic message to UI immediately
      addOptimisticMessage(optimisticMessage);

      try {
        // Send to server - on success, real messages will update via context
        await sendMessage(conversation.id, text);
        // No need to manually remove optimistic - useOptimistic auto-syncs when real state updates
      } catch (err) {
        // On error, the optimistic state automatically rolls back when real state doesn't change
