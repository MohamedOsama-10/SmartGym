...
const chatRequest = async (endpoint, options = {}) => {
  ...
  if (response.status === 403) {
    ...
    const error = new Error(err.detail || 'Not authorized');
    error.status = 403;
    throw error;
  }
...
export const ChatProvider = ({ children }) => {
  ...
  const loadMessages = useCallback(async (convId) => {
    ...
    try {
      ...
      // ← ADD THIS: also clear on 403 Forbidden (enhanced check)
      const isForbidden = err.status === 403 || 
                          err.message?.includes('Not authorized') ||
                          err.message?.includes('Forbidden') ||
                          err.message?.includes('403');
      if (isForbidden) {
        console.warn(`403 Forbidden for conversation ${convId}, clearing active conversation`);
        setActiveConversationId(null);
        sessionStorage.removeItem('chat_active_conv');
        return;
      }
  ...
  // ─── Owner: load a specific coach's inbox ────────────────────────────────
  const [viewingCoachId, setViewingCoachId] = useState(null);
  const [coachInboxConversations, setCoachInboxConversations] = useState([]);

  const loadCoachInbox = useCallback(async (coachUserId) => {
    if (!coachUserId) {
      setCoachInboxConversations([]);
      setViewingCoachId(null);
      return;
    }
    try {
      const data = await chatRequest(`/chat/coaches/${coachUserId}/conversations`);
      setCoachInboxConversations(data || []);
      setViewingCoachId(coachUserId);
    } catch (err) {
      console.error('Failed to load coach inbox:', err);
    }
  }, []);
  ...
  return (
    <ChatContext.Provider
      value={{
        ...
        isChatOpen,
        setIsChatOpen,
        // Owner inbox browsing
        viewingCoachId,
        coachInboxConversations,
        loadCoachInbox,
      }}
    >
      {children}
